...
st.set_page_config(
    page_title="CAS Sourcing MVP v13",
    page_icon="🧪",
    layout="wide",
)

st.title("🧪 CAS Sourcing & Procurement Intelligence MVP v13")
st.caption(
    "CAS input → supplier discovery → trusted catalog evidence → desired-quantity model → RFQ-ready shortlist. "
    "v13 adds exact public catalog matching, nearest-pack interpolation, supplier-specific parser profiles, and supplier coverage reporting."
)
...
with st.sidebar:
    ...
    if data_mode == "Live supplier discovery":
        max_pages = st.slider("Max total pages to extract", min_value=6, max_value=120, value=72)
        max_suppliers = st.slider("Max suppliers to walk", min_value=6, max_value=36, value=36)
...
def render_price_and_bulk_sections(ranked: pd.DataFrame, desired_qty_g: float, desired_quantity: float, desired_unit: str) -> None:
    ...
    if visible.empty:
        st.warning(
            "No visible prices were extracted by the current parser profiles. This usually means the supplier hides pricing behind login, JavaScript, account-specific contracts, or quote gates. "
    ...
    evidence_cols = [c for c in [
        ...
        "trust_warning", "supplier_parser_name", "supplier_parser_status", "product_url"
    ...
    st.write(
        ...
        f"Effective v13 exponent: **{best['effective_total_price_exponent']:.2f}**. "
    ...
    fig2 = px.bar(
        ...
        title="v13 Desired-Quantity Estimate by Supplier/Product",
    )
    ...
    st.download_button(
        ...
        data=quantity_csv,
        file_name=f"cas_quantity_model_v13_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
...
if run_search:
    ...
    if data_mode == "Live supplier discovery" and "supplier_summary_df" in locals() and not supplier_summary_df.empty:
        summary_cols = [c for c in ["supplier", "cas_number", "cas_exact_match", "products_found", "catalog_numbers", "purities_found", "product_forms", "pack_options", "visible_price_count", "bulk_estimate_eligible_count", "best_visible_price_usd", "price_visibility_status", "best_action", "stock_summary", "max_extraction_confidence", "high_confidence_price_pairs", "medium_confidence_price_pairs", "trust_decisions", "supplier_parser_names", "supplier_parser_statuses", "representative_url"] if c in supplier_summary_df.columns]
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"cas_product_evidence_v13_{cas_number.replace('-', '_')}.csv",
        mime="text/csv",
    )
    if data_mode == "Live supplier discovery" and "supplier_summary_df" in locals() and not supplier_summary_df.empty:
        ...
        st.download_button(
            ...
            data=summary_csv,
            file_name=f"cas_supplier_summary_v13_{cas_number.replace('-', '_')}.csv",
            mime="text/csv",
        )
    ...
    if data_mode == "Live supplier discovery" and "supplier_coverage_df" in locals() and not supplier_coverage_df.empty:
        ...
        st.download_button(
            ...
            data=coverage_csv,
            file_name=f"cas_supplier_coverage_v13_{cas_number.replace('-', '_')}.csv",
            mime="text/csv",
        )
else:
    ...
    st.markdown(
        ...
        v13 keeps the baseline intact and adds supplier-specific parser profiles, coverage reporting, exact catalog match logic, and nearest-pack interpolation before any scale-up model.
        """
    )
